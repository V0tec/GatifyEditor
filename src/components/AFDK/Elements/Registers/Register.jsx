...
function Register({
  ...
  // Визначаємо назву регістра
  const registerName =
    {
      REGISTER_4BIT: "REG 4-bit",
      REGISTER_8BIT: "REG 8-bit",
      SHIFT_REGISTER_4BIT: "SHIFT 4-bit",
      SHIFT_REGISTER_8BIT: "SHIFT 8-bit",
    }[component.type] || "Register";

  // Формуємо значення регістра для відображення
  let displayValue = "";
  if (component.state) {
    const bits = [];
    for (let i = 0; i < 8; i++) {
      if (component.state[`Q${i}`] !== undefined) {
        bits.push(component.state[`Q${i}`]);
      }
    }
    const binaryValue = bits.reverse().join("");
    const decimalValue = parseInt(binaryValue, 2) || 0;
    displayValue = `${binaryValue}₂ = ${decimalValue}₁₀`;
  }
