...
function Counter({
  ...
  const counterName =
    {
      COUNTER_4BIT: "CNT 4-bit",
      COUNTER_8BIT: "CNT 8-bit",
      COUNTER_4BIT_UP: "CNT 4-bit UP",
    }[component.type] || "Counter";

  // Формуємо значення лічильника для відображення
  let displayValue = "";
  if (component.state) {
    const count = component.state.count ?? 0;
    const bitCount = component.type === "COUNTER_8BIT" ? 8 : 4;
    const binaryValue = count.toString(2).padStart(bitCount, "0");
    displayValue = `${binaryValue}₂ = ${count}₁₀`;
  }
