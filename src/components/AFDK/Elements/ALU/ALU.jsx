...
function ALU({
  ...
  // Формуємо відображення операції та результату
  let displayInfo = "";
  if (component.inputs && component.outputs) {
    const bitCount = component.type === "ALU_4BIT" ? 4 : 8;
    ...
    // Читаємо операцію (OP0-OP2)
    const OP = component.inputs
      .slice(bitCount * 2, bitCount * 2 + 3)
      .map((inp) => {
        // Якщо не підключений - завжди 0
        if (!inp.connected) return 0;

        // Інакше читаємо значення з урахуванням інвертування
    ...
    const opName = opNames[operation] || "???";

    // Читаємо результат
    const R = component.outputs
      .slice(0, bitCount)
      .map((out) => out.value ?? 0)
      .reverse()
      .join("");
    const numR = parseInt(R, 2) || 0;

    displayInfo = `${opName}: ${numA} → ${numR}`;
  }

  return (
    <div
      className={styles.alu}
